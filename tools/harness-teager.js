...
const MOD = process.argv[2] || 'modules/teager.html';
...
if (require.main === module) {
  setTimeout(() => {
    ...
    const IDS = ['s1u', 's1e', 's1t', 's1f', 's1c', 's2a', 's2b', 's2c', 's2d', 's2e',
      's3a', 's3b', 's3c', 's3d', 's3e', 's4a', 's4b', 's4c', 's4d', 's4e', 's4f',
      's5a', 's5b', 's5c', 's5d', 's5e', 's5f',
      's6w', 's6a', 's6b', 's6c', 's6d', 's6e'];

    function dump(label) {
      M.drawAll();
      const S = M.state();
      console.log('\n== ' + label + '  ' + JSON.stringify({
        freq: S.freq, decay: S.decay, noise: S.noise, cut: S.cut,
    ...
    console.log('\n--- shared library vs local fallback ---');
    {
      ...
      let worst = 0;
      [['tkDiscrete', (L) => L.tkDiscrete(u)],
       ['tkEnergy holo', (L) => L.tkEnergy(u, M.DT, 'holo')],
       ['tkAnalytic', (L) => L.tkAnalytic(u, M.DT, 'holo')],
       ['tkVariation', (L) => L.tkVariation(L.tkAnalytic(u, M.DT, 'holo'), M.DT,
          { f1: 6, f2: 10, f3: 70, f4: 90 })],
       ['avt', (L) => L.avt(u, M.DT, 12).avt]].forEach((q) => {
    ...
    dump('defaults');
    const S = M.state();
    S.cut = 0; M.set('cut', 0); dump('TKV filter off');
    S.cut = 14; M.set('cut', 14); dump('TKV filter 14 Hz');
    S.cut = 6; M.set('cut', 6);
    ...
    S.freq = 45; M.set('freq', 45); dump('45 Hz');
