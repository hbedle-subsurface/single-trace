...
const MOD = process.argv[2] || 'modules/agc.html';
...
if (require.main === module) {
  setTimeout(() => {
    ...
    const IDS = ['s1w', 's1u', 's1r', 's1x', 's1g', 's1a',
      's2a', 's2b', 's2c', 's2d', 's2e', 's2r', 's2s',
    ...
    function dump(label) {
      ...
      console.log('\n== ' + label + '  ' + JSON.stringify({
        win: S.win, cmp: S.cmp, decay: S.decay, freq: S.freq, noise: S.noise,
    ...
    dump('defaults');
    const S = M.state();
    S.win = 10; M.set('win', 10); dump('window 10 ms');
    S.win = 300; M.set('win', 300); dump('window 300 ms');
    S.win = 120; M.set('win', 120);
    ...
    S.decay = 24; M.set('decay', 24); dump('decay 24 dB/s');
    S.decay = 14; M.set('decay', 14);
